...
class FunctionsPrint_2x {
  ...
  public static function formatFactDate(Fact $event, GedcomRecord $record, $anchor, $time) {
    ...
    if (preg_match('/\n2 DATE (.+)/', $factrec, $match)) {
      ...
      if ($record instanceof Individual) {
        ...
        $indi = Individual::getInstance($pid, $record->tree());
        if ($indi) {
          ...
          if (Date::compare($date, $death_date) <= 0) {
            ...
            if ($age != '' && $age > 0) {
              if (
                      $fact_age != '' && $fact_age != $age ||
                      $fact_age == '' && $husb_age == '' && $wife_age == '' ||
                      $husb_age != '' && $indi->sex() == 'M' && $husb_age != $age ||
                      $wife_age != '' && $indi->sex() == 'F' && $wife_age != $age
              ) {
                $ageText = '(' . I18N::translate('Age') . ' ' . FunctionsDate::getAgeAtEvent($age) . ')';
              }
            }
  ...
  protected function formatPlaceNotes(PlaceStructure $place) {
    ...
    if (preg_match('/\d NOTE (.*)/', $place->getGedcom(), $match)) {
      $html .= '<br>' . self::printFactNotes($place->getGedcom(), 3);
    }
    return $html;
  }
