...
class FunctionsFactPlace {
    ...
    public function mapCoordinates(
        ...
        if ($mapCoordinates !== null) {
          ...
          if ($hideCoordinates) {
            $map_lati = $mapCoordinates->getLati();
            $map_long = $mapCoordinates->getLong();
            
            $map_lati = trim(strtr($map_lati, "NSEW,�", " - -. ")); // S5,6789 ==> -5.6789
            $map_long = trim(strtr($map_long, "NSEW,�", " - -. ")); // E3.456� ==> 3.456            
            
            $html .= $this->getMapLinks(
                    $map_lati, 
                    $map_long,
                    $debug);
          } else {
            $html .= $this->formatPlaceLatiLong(
                    $mapCoordinates->getLati(), 
                    $mapCoordinates->getLong(),
                    $debug, 
                    $mapCoordinates->getTrace()->getAll());
          }
        }
    ...
    protected function formatPlaceLatiLong(
            $map_lati, 
            $map_long, 
            string $debugMapLinks,
            $tooltip = null): string {
        ...
        if ($map_lati && $map_long) {
            ...
            $mapLinks = $this->getMapLinks($map_lati, $map_long, $debugMapLinks);            
            $html .= $mapLinks;
